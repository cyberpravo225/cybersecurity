# КиберПроверка (new standalone game)

Этот репозиторий содержит веб-проект по кибербезопасности с новой отдельной игрой **КиберПроверка** (не как часть Binary Battle):
...
1. Главная страница (`index.html`)
   - добавлена большая карточка КиберПроверка;
