  ...
  const cards = [
    {
      ...
      description: 'Старший уровень «Разбери письмо»: подмена домена, социальная инженерия, сбор IOC и решение об эскалации.',
      challenge: {
        mode: 'multi',
    ...
    },
    {
      ...
      description: 'Продвинутая версия «Проверь сайт»: анализ URL, логики формы, давления сроками и запроса лишних данных.',
      challenge: {
        mode: 'multi',
    ...
    },
    {
      ...
      description: 'Сценарная мини-симуляция реагирования: от первого сигнала до координации команды и сохранения логов.',
      challenge: {
        mode: 'single',
        intro: 'Три учётки учителей скомпрометированы после рассылки «Обновите расписание».',
        question: 'Что нужно сделать в первую очередь?',
        answers: [
          { text: 'Удалить все письма и очистить логи, чтобы «не пугать пользователей».', correct: false, explain: 'Так теряются артефакты и усложняется расследование.' },
          { text: 'Изолировать учётки, завершить сессии, принудительно сменить пароли и включить MFA.', correct: true, explain: 'Первый этап — containment и остановка развития атаки.' },
          { text: 'Сразу отправить пресс-релиз в соцсети школы.', correct: false, explain: 'Коммуникации важны, но не заменяют оперативного containment.' },
          { text: 'Подождать до конца уроков, чтобы не мешать занятиям.', correct: false, explain: 'Задержка повышает риск дальнейшего распространения.' }
        ],
        success: 'Точно: после containment команда переходит к сбору логов, оценке масштаба и обновлению playbook.'
      }
    },
    {
      id: 'password-policy-lab',
      type: 'lab',
      icon: '🔐',
      title: 'Лаба: Политика паролей и MFA для школы',
      description: 'Спроектируй минимальный baseline защиты аккаунтов для учеников, учителей и админов.',
      challenge: {
        mode: 'multi',
        intro: 'Школа запускает новый цифровой кабинет и просит правила безопасности для учётных записей.',
        question: 'Какие меры должны войти в базовую политику?',
        answers: [
          { text: 'Уникальные пароли для разных сервисов + запрет повторов старых паролей.', correct: true, explain: 'Уникальность снижает риск каскадных взломов после утечки.' },
          { text: 'MFA обязательна минимум для учителей и администраторов.', correct: true, explain: 'MFA снижает вероятность успешного захвата учётки.' },
          { text: 'Хранить список паролей в общем чате класса для «резервного доступа».', correct: false, explain: 'Это критическое нарушение безопасности.' },
          { text: 'Блокировка после серии неудачных попыток входа.', correct: true, explain: 'Так снижается риск brute-force атак.' },
          { text: 'Использовать один общий аккаунт на весь класс.', correct: false, explain: 'Общий аккаунт уничтожает персональную ответственность и аудит.' }
        ],
        success: 'Отлично: получился рабочий baseline уровня школы с акцентом на реальную кибергигиену.'
      }
    },
    {
      ...
      description: 'Уровень 9–11: чат + почта + соцсети + подмена контекста переписки. Решай как аналитик, а не как пользователь.',
      challenge: {
    ...
    },
    {
      ...
      description: 'Расширенная стратегия blue team: быстрое containment, минимизация ущерба и координация ролей.',
      challenge: {
    ...
    },
    {
      ...
      description: 'Найди аномалию среди нормальной активности: география IP, скорость логинов и поведенческие маркеры.',
      challenge: {
        intro: 'Ты дежурный аналитик. Нужно быстро отличить ложную тревогу от автоматизированной атаки.',
        question: 'Какой набор событий наиболее опасен?',
        answers: [
          { text: 'Один ученик ошибся паролем и через минуту вошёл успешно.', correct: false, explain: 'Обычный сценарий пользователя.' },
          { text: '120 попыток входа в одну учётку + новые IP из разных регионов за 3 минуты.', correct: true, explain: 'Похоже на credential stuffing и требует немедленных мер.' },
          { text: 'Ночной бэкап и обновление сигнатур антивируса.', correct: false, explain: 'Штатная техническая активность.' },
          { text: 'Пользователь вошёл с нового школьного ноутбука после смены кабинета.', correct: false, explain: 'Изменение устройства не всегда означает атаку.' }
        ],
        success: 'Верно: фиксируй IOC, запускай блокировку и уведомляй команду по протоколу.'
      }
    },
    {
      id: 'red-vs-blue-game',
      type: 'game',
      icon: '🎮',
      title: 'Игра: Red vs Blue — тактический выбор',
      description: 'Выбери лучший контршаг против действий атакующего: от фишинга до lateral movement.',
      challenge: {
        intro: 'Сценарий: атакующий получил один аккаунт ученика и пытается расширить доступ.',
        question: 'Какой защитный шаг наиболее эффективен на старте?',
        answers: [
          { text: 'Оставить всё как есть, но попросить класс быть внимательнее.', correct: false, explain: 'Без технических мер атака может продолжиться.' },
          { text: 'Ограничить права скомпрометированной учётки, завершить сессии и включить принудительную смену пароля.', correct: true, explain: 'Это прямой шаг к локализации угрозы.' },
          { text: 'Удалить весь журнал событий за день.', correct: false, explain: 'Так исчезают улики и теряется хронология.' },
          { text: 'Отключить интернет во всей школе на неделю.', correct: false, explain: 'Несоразмерная мера без анализа создаёт лишний ущерб.' }
        ],
        success: 'Отличная тактика: минимизируешь распространение и сохраняешь контроль над инцидентом.'
      }
    }
  ];
